...
{ 0, };
int16_t dma_transmit_buffer[DMA_BUFFER_SIZE] =
{ 0, };
int16_t dma_receive_buffer[DMA_BUFFER_SIZE] =
{ 0, };
...
HAL_StatusTypeDef aic3204_write(uint8 addr, uint8 dat)
{
	...
	tmp[0] = dat;
	res = HAL_I2C_Mem_Write(&hi2c1, AIC3204_I2C_ADDRESS, (uint16_t) addr,
	I2C_MEMADD_SIZE_8BIT, tmp, 1, 100);
...
{
	HAL_StatusTypeDef res;
	res = HAL_I2C_Mem_Read(&hi2c1, AIC3204_I2C_ADDRESS, (uint16_t) addr,
	I2C_MEMADD_SIZE_8BIT, dat, 1, 100);
	return (res);
}
...
void I2C_Read(uint8_t id)
{
	...
	rst = HAL_I2C_Mem_Read(&hi2c1, AIC3204_I2C_ADDRESS, id, 1, rw_data, 1,
			10000);
	if (rst == HAL_OK)
...
void I2C_Write(uint8_t id, uint8_t dat)
{ // id : slave register address , 1 : address byte  , dat : buffer address
	uint8_t tmp[2];
	tmp[0] = dat;
...
{
	wt_data[0] = dat;
	rst = HAL_I2C_Mem_Write(&hi2c1, AIC3204_I2C_ADDRESS, id, 1, wt_data,
			sizeof(wt_data), 10000);
}
...
void process_audio_transmit_data(int16 *audio_buffer, uint16_t size)
{
	...
	{
		if (size == DMA_BUFFER_SIZE / 2)
		{ //RxHalfCpltCallback , tx_flag = 1
			for (int i = 0; i < DMA_BUFFER_SIZE / 2; i++)
			{
				dma_transmit_buffer[i] = audio_buffer[i
						+ tx_flag * DMA_BUFFER_SIZE];
			}

		}
		else
		{ //HAL_I2S_RxCpltCallback , tx_flag = 1
			for (int i = DMA_BUFFER_SIZE / 2; i < DMA_BUFFER_SIZE; i++)
			{
				dma_transmit_buffer[i] = audio_buffer[i
						+ tx_flag * DMA_BUFFER_SIZE];
...
void process_audio_receive_data(int16_t *dma_receive_buffer, uint16_t size)
{
	...
	{
		if (size == DMA_BUFFER_SIZE / 2)
		{ //RxHalfCpltCallback , rx_flag = 1
			for (int i = 0; i < DMA_BUFFER_SIZE / 2; i++)
			{
				audio_buffer[i + rx_flag * DMA_BUFFER_SIZE] =
						dma_receive_buffer[i];
			}

		}
		else
		{ //HAL_I2S_RxCpltCallback , rx_flag = 1
			for (int i = DMA_BUFFER_SIZE / 2; i < DMA_BUFFER_SIZE; i++)
			{
				audio_buffer[i + rx_flag * DMA_BUFFER_SIZE] =
						dma_receive_buffer[i];
...
void flash_write_page(uint8_t *pBuffer, uint32_t Page_Address,
		uint32_t OffsetInByte, uint32_t NumByteToWrite_up_to_PageSize)
{
	if (((NumByteToWrite_up_to_PageSize + OffsetInByte) > PageSize)
...
void flash_write_sector(uint8_t *pBuffer, uint32_t Sector_Address,
		uint32_t OffsetInByte, uint32_t NumByteToWrite_up_to_SectorSize)
{
	if ((NumByteToWrite_up_to_SectorSize > SectorSize)
...
void compare_flash_read(uint32_t Bytes_Address)
{ // Read(0Ch)
	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_5, RESET); // CS to low
	flash_spi(0x0C);
	flash_spi((Bytes_Address & 0xFF000000) >> 24);
...
void flash_read(uint32_t Bytes_Address)
{ // Read(0Ch)
	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_5, RESET); // CS to low
	flash_spi(0x0C);
	flash_spi((Bytes_Address & 0xFF000000) >> 24);
...
void sector_backup(uint32_t Bytes_Address)
{ // Read(03h)
	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_5, RESET); // CS to low
	flash_spi(0x0C);
	flash_spi((Bytes_Address & 0xFF000000) >> 24);
...
{ // Read Status Register (05h - 1) (35h - 2)
	uint8_t status = 0;
	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_5, RESET); // CS to low
	if (register_num == 1)
	{
...
//					flash_add = GetSector(hex2); //sector explorer
			...
			{
				if ((frame2 != NULL)
						&& (!strcmp(frame2, "wt") || !strcmp(frame2, "WT")))
				{
					...
					PRINTF_DEBUG(">>");
						...
						&& (!strcmp(frame2, "rd") || !strcmp(frame2, "RD")))
				{
					hex0 = StringToHexa(frame3);
					I2C_Read(hex0);
					PRINTF_DEBUG(">>");
						...
						&& (!strcmp(frame2, "init") || !strcmp(frame2, "INIT")))
				{
					if (dma_flag == 3)
					{
						dma_flag = 0;
					...
					&& (!strcmp(frame2, "mset") || !strcmp(frame2, "MSET")))
			{
...
//		/*Receive_Buf tx_rx*/
		if (dma_flag == 2)
		{
			HAL_I2S_Transmit_DMA(&hi2s2, (uint16*) dma_transmit_buffer,
			DMA_BUFFER_SIZE);
			dma_flag++;
		}
		/* USER CODE END WHILE */
...
static void MX_GPIO_Init(void)
{
	...
	HAL_GPIO_WritePin(GPIOC, FLASH_SS_Pin | FND_COM1_Pin | FND_COM2_Pin,
			GPIO_PIN_RESET);

	/*Configure GPIO pin Output Level */
	HAL_GPIO_WritePin(GPIOB,
			SEG_A_Pin | SEG_B_Pin | SEG_F_Pin | SEG_G_Pin | SEG_DP_Pin
					| GPIO_PIN_3 | SEG_C_Pin | SEG_D_Pin | SEG_E_Pin,
			GPIO_PIN_RESET);
	...
	GPIO_InitStruct.Pin = SEG_A_Pin | SEG_B_Pin | SEG_F_Pin | SEG_G_Pin
			| SEG_DP_Pin | GPIO_PIN_3 | SEG_C_Pin | SEG_D_Pin | SEG_E_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
